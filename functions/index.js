...
app.get('/api', (req, res) => {
  ...
  req_bongs: JSON.stringify(req.headers)});
  //res.json({'req': req});
});
*/

exports.bigBen = functions.https.onRequest(app.get('/api/bigbenUrl', (req, res) => {
  const date = new Date();
  const hours = (date.getHours() % 12) + 1;  // London is UTC + 1hr;
  console.log(JSON.stringify(req.headers))
  res.send(`
...
exports.api = {
  bigBen: exports.bigBen
}

  
exports.api2 = functions.https.onRequest(app.get('/dddddd', (req, res) => {
  const date = new Date();
  const hours = (date.getHours() % 12) + 1;  // London is UTC + 1hr;
  console.log(JSON.stringify(req.headers))
  res.json({bongs: 'BONG '.repeat(hours),
